...
from consumer.core.verify import unverified
from tests.conftest import CIPHER, StaticKeyProvider, encrypt_bytes, flip_byte
...
def test_chunked_round_trip(tmp_path: Path, key: bytes, package: bytes) -> None:
    ...
    decrypt_file(unverified(src), tmp_path / "out" / "model.tar", StaticKeyProvider(key))
...
def test_one_shot_round_trip(tmp_path: Path, key: bytes, package: bytes) -> None:
    ...
    decrypt_file(unverified(src), tmp_path / "model.tar", StaticKeyProvider(key))
...
def test_wrong_key_fails_and_leaves_no_plaintext(tmp_path: Path, artifact: bytes) -> None:
    ...
    with pytest.raises(DecryptError, match="decryption failed"):
        decrypt_file(unverified(src), tmp_path / "model.tar", StaticKeyProvider(os.urandom(32)))
...
def test_tampered_artifact_fails(
    ...
    src = tmp_path / "model.enc"
    src.write_bytes(flip_byte(artifact, index))
