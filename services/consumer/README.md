...
Downloads the encrypted model artifact from the Hugging Face Hub, decrypts it with the
Layer 1 key (a Kubernetes Secret mounted as a file), restores the model package safely,
loads it with Transformers and runs one fill-mask inference. Runs as a Kubernetes `Job`.
...
Flags override the environment: `--artifact-name`, `--revision`, `--key-source file|env`,
...
| `CONSUMER_ARTIFACT_NAME` | `model.enc` | encrypted file name |
| `CONSUMER_ARTIFACT_REVISION` | `main` | Hub revision to fetch |
...
docker run --rm -v "$PWD/var/secrets/model.key:/etc/model-key/key:ro" consumer:dev --repo-id <user>/<repo>
```
